...
export const hammerheadGetRoutes = async (
  ...
  for (let page = 1; ; page++) {
    var routesResponse = await fetch(
      `https://dashboard.hammerhead.io/v1/users/${hammerheadAuthorization.userId}/routes?per_page=${pageSize}&page=${page}`,
      {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${hammerheadAuthorization.accessToken}`,
...
export const hammerheadUploadRoute = async (
  ...
  var uploadResponse = await fetch(
    `https://dashboard.hammerhead.io/v1/users/${hammerheadAuthorization.userId}/routes/import/file`,
    {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${hammerheadAuthorization.accessToken}`,
